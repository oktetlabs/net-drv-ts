...
/**
 * @defgroup ethtool-dump_module_eeprom Obtain module EEPROM dump
 * @ingroup ethtool
 * @{
 *
 * @objective Check that module EEPROM dump can be obtained.
 *
...
#define TE_TEST_NAME "ethtool/dump_module_eeprom"
...
main(int argc, char *argv[])
{
    ...
    TEST_STEP("Obtain module EEPROM dump with Ethtool on @p iut_if.");
    opts.cmd = TAPI_ETHTOOL_CMD_DUMP_MODULE_EEPROM;
    ...
    if (report.err_out)
    {
        ...
        {
            TEST_VERDICT("Getting module EEPROM dump is not supported");
        }
        else
        {
