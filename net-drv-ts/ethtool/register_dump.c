...
/**
 * @defgroup ethtool-register_dump Obtain register dump
 * @ingroup ethtool
 * @{
 *
 * @objective Check that register dump can be obtained.
 *
...
#define TE_TEST_NAME "ethtool/register_dump"
...
main(int argc, char *argv[])
{
    ...
    TEST_STEP("Obtain register dump with Ethtool on @p iut_if.");
    opts.cmd = TAPI_ETHTOOL_CMD_REG_DUMP;
    ...
    if (report.err_out)
    {
        ...
        {
            TEST_VERDICT("Getting register dump is not supported");
        }
        else
        {
