...
/**
 * @defgroup ethtool-show_fec Show FEC parameters with ethtool
 * @ingroup ethtool
 * @{
 *
 * @objective Check that ethtool can show FEC parameters for
 *            a tested interface.
 *
...
#define TE_TEST_NAME "ethtool/show_fec"
...
main(int argc, char *argv[])
{
    ...
    TEST_STEP("Run 'ethtool \\--show-fec @p iut_if'. Check that it "
              "terminates successfully, does not print anything to "
              "stderr and its output can be parsed.");

    opts.cmd = TAPI_ETHTOOL_CMD_SHOW_FEC;
