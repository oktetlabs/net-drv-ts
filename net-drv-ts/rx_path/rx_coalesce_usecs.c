...
main(int argc, char *argv[])
{
    ...
    TEST_STEP("Set @b rx_coalesce_usecs to the value of @p coalesce_usecs "
              "for the IUT interface. Also for nonzero @p coalesce_usecs "
              "set @b rx_max_coalesced_frames to zero so that it is not "
              "taken into account when coalescing; for zero "
              "@p coalesce_usecs set it to @c 1.");
